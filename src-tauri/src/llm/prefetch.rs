...
mod tests {
    ...
    async fn test_search_by_path_substring_parameterized() {
        ...
        let count = engine.sqlite.count().await.unwrap();
    ...
    async fn test_bm25_search_returns_relevant_sorted() {
        ...
        engine.sqlite.insert(&m1).await.unwrap();
        engine.sqlite.insert(&m2).await.unwrap();
        engine.sqlite.insert(&m3).await.unwrap();
    ...
    async fn test_vector_search_filters_chat_channel() {
        ...
        engine.sqlite.insert(&m_other).await.unwrap();
        engine.lance.upsert("vec-user", &vec).await.unwrap();
        engine.lance.upsert("vec-asst", &vec).await.unwrap();
        engine.lance.upsert("vec-other", &vec).await.unwrap();
