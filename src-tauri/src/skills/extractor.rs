...
const EXTRACTION_SYSTEM_PROMPT: &str = r#"You are the Skill Extractor for nebula, a multi-agent desktop AI assistant.
...
impl SkillExtractor {
    ...
    pub async fn try_extract(
        ...
        match llm_result {
            Err(e) => {
                warn!(target: "nebula.skills", error = %e, "skill extraction LLM call failed");
                ExtractionReport {
                    task_description: task_description.to_string(),
                    extractable: false,
                    skill_name: None,
            ...
            Ok(response) => {
                ...
                match serde_json::from_str::<ExtractionResult>(json_str) {
                    Err(e) => {
                        warn!(target: "nebula.skills", error = %e, body = %json_str, "failed to parse extraction result");
                        ExtractionReport {
                            task_description: task_description.to_string(),
                            extractable: false,
                            skill_name: None,
                    ...
                    Ok(extracted) => {
                        if !extracted.extractable {
                            info!(target: "nebula.skills", task = %task_description, "task not extractable");
                            return ExtractionReport {
                                task_description: task_description.to_string(),
                                extractable: false,
                                skill_name: None,
                        ...
                        match self.persist_skill(&extracted).await {
                            ...
                            Err(e) => {
                                warn!(target: "nebula.skills", error = %e, name = %extracted.name, "failed to persist extracted skill");
                                ExtractionReport {
                                    task_description: task_description.to_string(),
                                    extractable: true,
                                    skill_name: Some(extracted.name.clone()),
...
> Auto-generated by nebula Skill Extractor (v1.2).
...
"#,
        ...
        info!(target: "nebula.skills", path = %path.display(), "wrote SKILL.md");
