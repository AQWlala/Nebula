...
pub use templates::{WritingTemplate, TemplatePlaceholder};
...
async fn mirror_to_l3(
    ...
    if let serde_json::Value::Object(ref mut map) = mem.metadata {
        map.insert("document_id".to_string(), serde_json::Value::from(document_id));
        map.insert("template_id".to_string(), serde_json::Value::from(template_id));
...
fn render_markdown(doc: &Document) -> String {
    ...
    out.push_str(&format!("<!-- generated by nine-snake v0.5, template={} -->\n\n", doc.template_id));
