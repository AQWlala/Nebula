...
**当前版本**：v2.0.0（M0a-M7b 全部里程碑 100% 完成）
...
| 单元测试 | 993 个（`#[test]` 698 + `#[tokio::test]` 295） |
| 集成测试 | 142 个 / 25 个测试文件 |
| 测试总计 | 1,500+（含 cfg-gated 模块不同 feature 组合） |
| 性能基准 | 3 个 criterion bench |
| ADR | 4 个（ADR-001/002/003/004,全部已接受/已实施） |
| 文档 | 28 个 markdown / ~12,000 行 |
| 里程碑 | 9 个（M0a-M7b）全部 ✅ 完成 |
| P0 修复 | 11/11 ✅ |
| P1 修复 | 22/22 ✅ |
...
- **Nebula 的记忆是可读+可编辑+可追溯的"信任记忆"**——行业唯一
...
| ADR | 标题 | 状态 | 核心决策 |
|-----|------|------|----------|
| ADR-001 | MasterOrchestrator 组合模式 | ✅ 已接受 | 委托 SwarmOrchestrator fan-out,持有 `Arc<SwarmOrchestrator>` 而非重复实现 Worker 池 |
| ADR-002 | TaskDag + petgraph | ✅ 已接受 | 引入 petgraph 0.6,DiGraph<SubTask, DependencyEdge>,零手写拓扑排序 |
| ADR-003 | UnifiedModelDispatcher | ✅ 已实施 v2.1 | 单一入口 `dispatch(WorkType, messages)`,双维度成本统计,4 Phase 迁移 |
| ADR-004 | Feature Flag 策略 | ✅ 已接受 | 4 个 feature flag 默认 off,双层 gate（Cargo feature + env var） |
...
| 阶段 | 状态 | 核心交付 |
|------|------|----------|
| M0a ADR-001/002 | ✅ | 4 个 ADR 编写 + v2.0 一致性修订 |
| M0b petgraph 引入 | ✅ | petgraph 0.6 + 4 feature flag + ADR-004 |
| M0c P0 修订 + Dispatcher 骨架 | ✅ | 11 个 P0 全修复 + dispatcher.rs 骨架 + 30 测试 |
| M1 Soul 系统 | ✅ | SoulCompiler 6 Step + 双扫描 + 原子写入 + 33 测试 |
| M2a domain schema | ✅ | Memory.domain 字段 + migration 035 + 113 测试 |
| M2b ACL 重写 | ✅ | MemoryAcl v2 + PrincipalDomainMap + 128 测试 |
| M3 MasterOrchestrator + DAG | ✅ | TaskDag + ExecuteMode + WorkType 精简 7 变体 |
| M4 EvolutionEngine | ✅ | 4 Phase pipeline + 三层共存 + 回滚 + 28 测试 |
| M5 L4 审批 + 流式 | ✅ | ApprovalGate + CostPolicy + chat_stream + 97+16 测试 |
| M6 前端 | ✅ | SoulEditor + EvolutionLogView + DagCanvas + WorkTypeConfigView + i18n 双语 |
| M7a chat 迁移 | ✅ | chat → dispatch(WorkType::Chat) + criterion bench |
| M7b 集成测试 + 发布 | ✅ | 1339 单测全绿 + 26 安全缺口修复 + 发布就绪 |
...
| 文档 | 内容 | 状态 |
|------|------|------|
| [WHITEPAPER_v3.1.md](docs/WHITEPAPER_v3.1.md) | **v3.1 创新白皮书 + 实施完成总结** ⭐ | ✅ 当前权威 |
| [WHITEPAPER_v2.0.md](docs/WHITEPAPER_v2.0.md) | 基础架构权威（§1-§17） | ✅ 基础架构 |
| [PRODUCTION_TASK_TRACKER.md](docs/PRODUCTION_TASK_TRACKER.md) | M0a-M7b 生产任务追踪表 | ✅ 100% 完成 |
| [ADR-001](docs/ADR-001-master-orchestrator-composition.md) | MasterOrchestrator 组合模式决策 | ✅ 已接受 |
| [ADR-002](docs/ADR-002-task-dag-petgraph.md) | TaskDag + petgraph DAG 决策 | ✅ 已接受 |
| [ADR-003](docs/ADR-003-unified-model-dispatcher.md) | UnifiedModelDispatcher 统一调度决策 | ✅ 已实施 v2.1 |
| [ADR-004](docs/ADR-004-feature-flag-strategy.md) | Feature Flag 策略决策 | ✅ 已接受 |
| [CHANGELOG.md](docs/CHANGELOG.md) | 版本变更日志（M0a-M7b 全部记录） | ✅ 最新 |
...
| [FEATURE_FLAG_AUDIT.md](docs/FEATURE_FLAG_AUDIT.md) | Feature flag 审计报告 | ✅ |
| [MIGRATION_ROLLBACK.md](docs/MIGRATION_ROLLBACK.md) | 数据库迁移回滚策略 | ✅ |
