...
| T-D-B-07 | **1,805 panic 点**（1,361 unwrap + 377 expect + 67 panic）→ ✅ 全部 `.unwrap()` → `.expect("msg")`（2026-07-09，production 0 unwrap，test 0 unwrap） | 后端 | L | v2.0 缺陷#1 |
...
| T-D-B-12 | 无自托管 Web 静态服务 → 🔄 WebStaticServer 已实现(SPA fallback+MIME+缓存+路径遍历防护+7测试),待集成到 rest.rs 路由 | 后端 | S | v2.0 部署 |
...
| T-D-F-04 | 硬编码中文字符串 → 迁移到 i18n key → 🔄 部分完成:export.ts(7处) + App.tsx(4处) 已迁移,autonomy.ts 数据结构 i18n 化待后续 | 前端 | M | v2.0 MI-06 |
...
| T-D-T-01 | vitest 覆盖率阈值过低（30%/20%/25%/30%）→ ✅ 已提升至 35/25/50/35,基于实测基线(Stmts 38.75%/Branch 59.31%/Funcs 28.51%/Lines 38.75%)留 3-9% 余量（2026-07-09） | 测试 | M | 外部审查 |
...
- ✅ T-D-B-07 1805 panic 点 → 全部 .expect("msg")（2026-07-09）
...
| 2026-07-09 | T-D-B-07 全部 `.unwrap()` → `.expect("msg")`：production 67→0，test 1471→0；cargo check 通过 → ✅ | §4.1, §6.2 |
