...
| T-E-L-05 | Loop 模板库 | ⚠️ 待标记 DONE |
| T-E-L-06 | Loop 预算管理 + 安全防护 | ⚠️ 待提交 |
...
## 4. 技术债务（41 个 T-D-* 任务，全部未开始）

> **来源**：代码质量审计（2026-07-08）+ 外部审查报告 + DEVELOPMENT_PROPOSAL_v2.0/v3.0 代码级审查。
> **任务编号**：`T-D-<领域>-<序号>`（F=前端 / B=后端 / C=CI配置 / T=测试 / S=安全配置 / O=仓库卫生）

### 4.1 P0 严重问题（13 个，阻塞构建/运行/安全）

| 任务 ID | 描述 | 领域 | 复杂度 | 来源 |
|---------|------|------|--------|------|
| T-D-B-15 | **🔴 digest crate 版本冲突**（sha2 v0.11 vs hkdf v0.13）→ 统一版本 | 后端 | S | v2.0 CR-01 |
| T-D-T-04 | **🔴 CI 集成测试被跳过**（25 个测试文件不执行）→ 隔离或加 Linux runner | 测试 | M | v2.0 CR-02 |
| T-D-B-16 | **🔴 AppState 膨胀：45+ Arc 字段巨型结构体** → 分组为 5-6 个 SubSystems | 后端 | L | v3.0 CR-04 |
| T-D-B-01 | tracing_setup.rs 8 路组合爆炸 → builder pattern | 后端 | S | 外部审查 |
| T-D-B-06 | **lib.rs 3,333 行巨型文件**（257 个 Tauri 命令）→ 拆分 | 后端 | L | v2.0 HI-01 |
| T-D-B-07 | **1,805 panic 点**（1,361 unwrap + 377 expect + 67 panic）→ < 50 | 后端 | L | v2.0 缺陷#1 |
| T-D-B-08 | **gRPC wire 非标准**（JSON framing shim）→ 迁移到 tonic | 后端 | L | v2.0 缺陷#3 |
| T-D-B-09 | **渠道路由层断路**（3 空操作适配器 + trait 缺陷 + 回信丢失） | 后端 | M | v2.0 渠道分析 |
| T-D-F-01 | tauri.ts 单文件 3190 行 → 按领域拆分 | 前端 | M | v2.0 HI-01 |
| T-D-C-01 | CI 仅 Windows → 恢复跨平台 matrix | CI | M | v2.0 HI-04 |
| T-D-F-02 | ESLint 配置不存在 → 新增 flat config | 前端 | S | v2.0 MI-01 |
| T-D-S-01 | cargo audit 14 个忽略 → 逐项评估 + 跟踪 | 安全 | M | v2.0 CR-03 |
| T-D-C-06 | 关键功能开关默认关闭 → 决策开启 | CI | S | v3.0 功能分析 |

### 4.2 P1 重要问题（21 个）

| 任务 ID | 描述 | 领域 | 复杂度 | 来源 |
|---------|------|------|--------|------|
| T-D-B-10 | Skill 生态补齐（发现层+规范层+导入层） | 后端 | M | v2.0 Skill 分析 |
| T-D-B-11 | EvolutionEngine 断路（evolution_run 未实现） | 后端 | L | v2.0 学习循环 |
| T-D-B-12 | 无自托管 Web 静态服务 | 后端 | S | v2.0 部署 |
| T-D-B-13 | 无系统服务注册（systemd/launchd/Windows Service） | 后端 | M | v2.0 部署 |
| T-D-B-14 | Sidecar 3/5 服务骨架化 | 后端 | M | v2.0 部署 |
| T-D-B-17 | **AgentKind Deprecated 但仍大量使用** → 统一为 Generic + 场景化 | 后端 | M | v3.0 HI-09 |
| T-D-B-18 | **writing/ 模块过于单薄（2 文件）** → 补齐 28 场景模板 | 后端 | L | v3.0 HI-10 |
| T-D-B-19 | **swarm/agents 6 角色偏编程场景** → 重定义为写作场景角色 | 后端 | M | v3.0 HI-11 |
| T-D-B-02 | bootstrap.rs 1113 行单函数 → 拆分 bootstrap phase | 后端 | L | v2.0 HI-02 |
| T-D-B-03 | std::mem::forget(h) 泄露 JoinHandle | 后端 | S | v2.0 MI-08 |
| T-D-B-04 | memory/ 40+ 子文件平铺 → 按职能分组 | 后端 | M | v2.0 HI-06 |
| T-D-B-05 | features 死 feature 清理 | 后端 | S | v2.0 MI-09 |
| T-D-F-03 | 重复代码提取（renderMarkdown / downloadBlob） | 前端 | S | 外部审查 |
| T-D-F-04 | 硬编码中文字符串 → 迁移到 i18n key | 前端 | M | v2.0 MI-06 |
| T-D-F-05 | i18n 类型不安全 → 基于 zh-CN.json 类型推导 | 前端 | S | 外部审查 |
| T-D-F-06 | cancelled 布尔反模式 → AbortController | 前端 | S | v2.0 MI-07 |
| T-D-T-01 | vitest 覆盖率阈值过低（30%/20%/25%/30%） | 测试 | M | 外部审查 |
| T-D-T-02 | 核心文件零测试（bootstrap/gateway/dispatcher/app_config） | 测试 | L | v2.0 HI-07 |
| T-D-T-03 | E2E 测试接入 CI（Playwright） | 测试 | M | 外部审查 |
| T-D-C-02 | Vite/Vitest 配置重复 → 统一单配置源 | CI | S | v2.0 MI-04 |
| T-D-C-03 | Prettier 配置不存在 → 新增 .prettierrc | CI | S | v2.0 MI-02 |
| T-D-C-04 | tsconfig 禁用 noUnusedLocals/Parameters | CI | S | v2.0 MI-03 |
| T-D-C-05 | Dockerfile 缺 HEALTHCHECK/非 root/多架构 | CI | M | v2.0 HI-08 |
| T-D-C-07 | incremental=false（Rust ICE 规避）→ 跟踪恢复 | CI | S | v2.0 HI-05 |
| T-D-C-08 | **master-orchestrator 无运行时开关** → 新增 AtomicBool | CI | S | v3.0 HI-12 |
...
| 任务 ID | 描述 | 领域 | 复杂度 | 来源 |
|---------|------|------|--------|------|
| T-D-O-01 | IMPROVEMENT_PLAN_v1.0.md 过时文件清理 | 仓库卫生 | S | 外部审查 |
| T-D-O-02 | **ARCHITECTURE.md 品牌残留与数字过时** → 统一为 Nebula | 仓库卫生 | S | v3.0 HI-13 |
...
4. **安全优先**：T-D-S-01/02 和 T-D-B-03 涉及安全，优先处理
5. **仓库卫生**：T-D-O-01/02 与 T-D-F-02（ESLint）一起做
...
1. **标记 T-E-L-05 为 DONE**（核实 3 commits 已落地）
2. **提交 T-E-L-06 工作区变更**（11 改 + 3 新增）并 push 触发 CI
3. **关闭 finish-te-l-03-checker-agent spec**
4. **🔴 T-D-B-15 digest crate 版本冲突修复**（构建阻塞，最先）
5. **🔴 T-D-T-04 CI 集成测试恢复**（回归风险真空）
