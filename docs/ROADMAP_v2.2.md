...
Stage 1-6（v2.1，工程闭环）
  ...
  ├─ Stage 2 协议+安全 ✅
  ├─ Stage 3 蜂群基础 ✅
  ├─ Stage 4 蜂群深度 ⏳（T-S4-B-01/02/03 待启动）
  ├─ Stage 5 UX 升级 ⏳（T-S5-A/B 待启动）
  └─ Stage 6 OS 集成 ⏳（T-S6-A/B 待启动）
                    │
                    ▼
...
| T-E-B-02 | **可读记忆三视图**：①Markdown 视图（可编辑笔记）②图谱视图（3D 可视化）③时间轴视图（`/journey` 回放） | P1 | XL | T-E-B-01 | B |
...
| T-E-B-07 | **知识图谱视图**：MemoryMap 升级为 Obsidian 风格力导向图（D3/PixiJS），支持 1000+ 节点 | P1 | L | 无 | A+B |
| T-E-B-08 | **Obsidian vault 兼容**：直接读取 `.obsidian/` 配置 + Markdown 双向同步 | P2 | M | T-E-B-03 | A |
...
| T-E-B-16 | **MDRM 5 维关系图谱**：扩展 CausalGraphEngine 为 5 维（因果/时序/实体/层级/相似度） | P2 | XL | 无 | B |
