...
| T-E-L-05 | ✅ DONE (2026-07-08) — **Loop 模板库**：7 种 Loop 模式的 LOOP.md 模板 + 复用 TemplatesDialog（新增 automation 类别，默认只露 2 个入口）。【3 commit 完成：①7 Loop mode LOOP.md templates(6396a0b) ②loop_templates_list/get 命令 + 6 回归测试(3a2b58c) ③前端 TemplatesDialog automation 类别(ad10a6a)】 | P2 | M | T-E-L-01 | Loop 内化 |
| T-E-L-06 | ✅ DONE (2026-07-08) — **Loop 预算管理 + 安全防护**：loop-budget.md（拆分本地 $0 / 云端两列）+ loop-cost 估算 + 超预算自动暂停 + loop-safety-guards.md（模型同质检测口径定义 + 自动降级触发条件）+ execute_loop 集成预算检查与同质检测门禁 + 3 个 Tauri 命令（loop_budget_status/reset/pause_all）。【1 commit 完成：18cf45e，含 LoopBudgetConfig/from_markdown()、CostTracker::monthly_cost_by_source/loop_cost_this_month、AppConfig loop_monthly_budget_usd/tokens、80%/100% 预算事件、master.rs execute_loop 接入、280 swarm tests passed】 | P2 | M | T-E-L-02 | Loop 内化 |
